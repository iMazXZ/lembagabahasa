@php
    ...
    $scores = is_array($certificate->scores ?? null) ? $certificate->scores : [];

    $finalTest = null;
    if (isset($scores['final_test']) && is_numeric($scores['final_test'])) {
        $finalTest = (float) $scores['final_test'];
    } elseif (isset($scores['score']) && is_numeric($scores['score'])) {
        $finalTest = (float) $scores['score'];
    } else {
        foreach ($scores as $value) {
            if (is_numeric($value)) {
                $finalTest = (float) $value;
                break;
            }
        }
    }

    $finalNumeric = is_numeric($certificate->average_score ?? null)
        ? (float) $certificate->average_score
        : $finalTest;
        ...
        <div class="scores-section avoid-break">
          <table class="scores-table">
            ...
            <tbody>
              <tr>
                <td>FINAL TEST</td>
                <td class="num-center">{{ $finalTest !== null ? number_format($finalTest, 2) : '—' }}</td>
              </tr>
              
              <tr class="grade-row">
                <td>FINAL SCORE</td>
                <td class="num-center">{{ $finalNumeric !== null ? number_format($finalNumeric, 2) : '—' }}</td>
              </tr>
              <tr class="grade-row">
                <td>GRADE</td>
